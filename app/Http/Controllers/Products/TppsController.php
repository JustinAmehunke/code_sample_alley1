        ...
        if (isset($request->doc_appl_id) && $request->doc_appl_id !== null){
            ...
            if($document_application !== null){
                try {
                    ...
                    $beneficiaryFields = [];
                    $coverFields = [];
                    $excludeFields = ['_token', 'doc_appl_id', 'rec_id', 'product_name', 'flexdatalist-occupation', 'flexdatalist-empoyment_occupation', 'flexdatalist-empoyment_employer', 'flexdatalist-agent_code', 'flexdatalist-employer', 'product_beneficiary','product_cover','product_signature'];
                    ...
                    if(isset($request->product_name)){
                        foreach ($request->all() as $field => $value) {
                            ...
                            if(!in_array($field, $excludeFields)){
                                if (!empty($value)) {
                                    if($field == "id_type_in_resident_country"){
                                        $customizedFields["id_type_not_ghana"] = $value;
                                    ...
                                    if($field == "id_type_in_resident_country"){
                                        $customizedFields["id_type_not_ghana"] = $document_application->$field;
                                    }else if($field == "id_number_in_resident_country"){
                        ...
                        Tpp::where('tbl_document_applications_id', $doc_id)->update($customizedFields);
                    }
                    ...
                    if(isset($request->product_cover)){
                        ...
                        if (!is_null($request->cover_surname_name) && (is_array($request->cover_surname_name) || is_object($request->cover_surname_name))) {
                            foreach ($request->cover_surname_name as $key => $fullName) {
                                    ...
                                    if(!in_array($field, $excludeFields)){
                                        ...
                                        if (isset($request[$field][$key ])) {
                                           if(is_array($request->$field)){
                                                $cover->$field = $request[$field][$key];
                                           }else{
                                                $cover->$field = $request->$field;
                                                // if($field != 'premium'){
                                                //     if($field == 'cover_one_premium'){
                                                //         $cover->cover_one_premium = $request->$field;
                                                //     }else{
                                                //         $cover->$field = $request->$field;
                                                //     }
                                ...
                                // return $cover;
                                //add custom fields
                                // $cover->cover_one_premium = $request->cover_one_premium;
                            ...
                            $cover = new Cover();
                            $cover->premium = $request->premium;
                            $cover->sum_assured = $request->sum_assured;
                            $cover->cover_one_premium = $request->cover_one_premium;
                    ...
                    if(isset($request->product_signature)){
                        ...
                        if($request->how_did_you_hear == "Self-Discovery"){
                            Tpp::where('tbl_document_applications_id', $doc_id)->update([
                        ...
                        $filled = Tpp::where('tbl_document_applications_id', $doc_id)->first();
